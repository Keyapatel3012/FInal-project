...
apod_desktop.init_apod_cache(apod_desktop.get_script_dir)
...
root.iconbitmap(os.path.join(apod_desktop.get_script_dir, 'NASA_logo.ico'))
...
frm_image.columnconfigure(0, weight=1)  # set weight to 1 to allow resizing
frm_image.rowconfigure(0, weight=1)  #set weight to 1 to allow resizing
frm_image.grid(row=0, columnspan=2, sticky=NSEW) 

# Create the frame to hold the image explanation
frm_explanation = ttk.Frame(root)
frm_explanation.columnconfigure(0, weight=1) # set weight to 1 to allow resizing
frm_explanation.rowconfigure(0, weight=1)# set weight to 1 to allow resizing
...
lblfrm_select.columnconfigure(0, weight=1) #set weight to 1 to allow resizing
...
lblfrm_download.columnconfigure(0, weight=1) #set weight to 1 to allow resizing
####################################
...
def handle_download_image():
    ...
    apod_id = apod_desktop.add_apod_to_cache(sel_date) # assuming  apod_to_cache takes the selected date
    if apod_id != 0:
